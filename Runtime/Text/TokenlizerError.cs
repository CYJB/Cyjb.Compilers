...
public class TokenlizerError
{
	...
	/// <summary>
	/// 使用词法分析错误的相关信息初始化 <see cref="TokenlizerError"/> 类的新实例。
	/// </summary>
	/// <param name="text">词法单元的文本。</param>
	/// <param name="span">词法单元的范围。</param>
	/// <param name="locator">行定位器。</param>
	public TokenlizerError(string text, TextSpan span, LineLocator? locator)
	{
		Text = text;
		Span = span;
	...
	public bool Equals(TokenlizerError other)
	{
	...
	{
		if (obj is TokenlizerError other)
		{
			return Equals(other);
		}
		return false;
	...
	public static bool operator ==(TokenlizerError left, TokenlizerError right)
	{
