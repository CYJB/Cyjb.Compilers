...
internal sealed class TokenlizerSimpler<T> : TokenlizerBase<T>
	where T : struct
{
	/// <summary>
	/// 使用给定的词法分析器信息初始化 <see cref="TokenlizerSimpler{T}"/> 类的新实例。
	/// </summary>
	/// <param name="lexerData">要使用的词法分析器的数据。</param>
	/// <param name="controller">词法分析控制器。</param>
	/// <param name="reader">要使用的源文件读取器。</param>
	public TokenlizerSimpler(LexerData<T> lexerData, LexerController<T> controller, SourceReader reader) :
