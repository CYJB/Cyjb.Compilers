...
/// <summary>
/// 表示带有调试信息的支持 Reject 动作的词法分析器核心。
/// </summary>
/// <typeparam name="T">词法单元标识符的类型，一般是一个枚举类型。</typeparam>
internal sealed class RejectableDebugCore<T> : LexerCore<T>
	where T : struct
{
	/// <summary>
	/// 词法分析器的数据。
	...
	public RejectableDebugCore(LexerData<T> lexerData, LexerController<T> controller) :
	...
	public override bool NextToken(int state, int start)
	{
		...
		while (true)
		{
			...
			if (lexerData.GetSymbols(state, ref symbolStart, ref symbolEnd))
			{
				if (lexerData.UseShortest)
				{
					...
					{
						var terminal = terminalData[states[i]];
		...
		PrintReadedText();
		return RunTerminalsDebug(start);
	}
}
